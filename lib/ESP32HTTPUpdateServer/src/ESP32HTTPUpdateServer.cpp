...
void ESP32HTTPUpdateServer::setup(WebServer *server, const String &path,
  ...
  _server->on(path.c_str(), HTTP_OPTIONS, [&]() {
    ...
    _server->send(200, "text/html", String("y"));
  });
      ...
      [&]() {
        ...
        if (Update.hasError()) {
          _server->send(200, "text/html",
                        String("Update error: ") + _updaterError);
        } else {
      ...
      [&]() {
        ...
        if (upload.status == UPLOAD_FILE_START) {
          ...
          if (upload.name == "filesystem") {
            // Для LittleFS на ESP32
            if (!Update.begin(
                    UPDATE_SIZE_UNKNOWN,
                    U_SPIFFS)) { // U_SPIFFS также работает для LittleFS
              if (_serial_output)
...
void ESP32HTTPUpdateServer::_setUpdaterError() {
  ...
  // Упрощенная обработка ошибки для ESP32
  _updaterError = "Update failed (check serial monitor for details)";
}
